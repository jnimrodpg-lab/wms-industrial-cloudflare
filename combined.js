...
console.info('*** REHYDRATION FIX ACTIVE ***');
  ...
  /* integrated expanded-card fixes: image fill + prev/next inside card */
  function shortProductNavName(name){
    const t = String(name || '').trim();
    return !t ? '—' : (t.length > 26 ? t.slice(0,26).trim() + '…' : t);
  }
  function getViewerCurrentList(){
  ...
  function getViewerProductGroups(){
    const list = getViewerCurrentList();
    const map = new Map();
    list.forEach((p) => {
  ...
  function getViewerCurrentGroupInfo(){
    const groups = getViewerProductGroups();
  ...
  function getGroupRepresentativeProduct(group){
  ...
  function ensureExpandedCardNav(){
    const card = document.getElementById('activeProductCard');
    if(!card) return null;
    ...
    if(prev && !prev.dataset.boundCardNav){
      prev.dataset.boundCardNav = '1';
      prev.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation();
        moveExpandedCardByProduct(-1);
      });
    }
    if(next && !next.dataset.boundCardNav){
      next.dataset.boundCardNav = '1';
  ...
  function renderExpandedNavButton(btn, arrow, kicker, name, side){
    if(!btn) return;
    btn.innerHTML = `
      ${side === 'left' ? `<span class="nav-arrow">${arrow}</span>` : ''}
      <span class="nav-text">
        <span class="nav-kicker">${kicker}</span>
        <span class="nav-name">${escapeHtml(name)}</span>
      </span>
      ${side === 'right' ? `<span class="nav-arrow">${arrow}</span>` : ''}
    `;
  }
  function updateExpandedCardNav(){
    const nav = ensureExpandedCardNav();
    if(!nav) return;
    const prev = nav.querySelector('#expandedCardPrevBtn');
    const next = nav.querySelector('#expandedCardNextBtn');
    const info = getViewerCurrentGroupInfo();
    const total = info.groups.length;
    if(total <= 1){
      renderExpandedNavButton(prev,'←','Anterior','—','left');
      renderExpandedNavButton(next,'→','Siguiente','—','right');
    ...
    if(prev){
      prev.disabled = false;
      renderExpandedNavButton(prev,'←','Anterior',shortProductNavName(prevGroup?.nombre),'left');
      prev.title = prevGroup?.nombre || 'Anterior';
    }
    if(next){
      next.disabled = false;
      renderExpandedNavButton(next,'→','Siguiente',shortProductNavName(nextGroup?.nombre),'right');
      next.title = nextGroup?.nombre || 'Siguiente';
    }
  }
  function moveExpandedCardByProduct(dir){
    const info = getViewerCurrentGroupInfo();
    ...
    const target = getGroupRepresentativeProduct(info.groups[nextIndex]);
    if(target && typeof selectProduct === 'function'){
      selectProduct(target);
      setTimeout(() => {
        if (typeof openActiveProductCard === 'function') openActiveProductCard();
        updateExpandedCardNav();
      }, 0);
    }
  }
  function injectExpandedCardFixStyles(){
    if (document.getElementById('expandedCardIntegratedFixStyle')) return;
    const style = document.createElement('style');
    style.id = 'expandedCardIntegratedFixStyle';
    style.textContent = `
      #activeProductCard .search-card-nav{ display:none; }
      ...
      #activeProductCard.search-card-expanded #expandedCardPrevBtn{ justify-content:flex-start !important; text-align:left !important; }
      #activeProductCard.search-card-expanded #expandedCardNextBtn{ justify-content:flex-end !important; text-align:right !important; }
      #activeProductCard.search-card-expanded .search-card-nav .nav-arrow{
        flex:0 0 auto !important;
        font-size:22px !important;
        font-weight:900 !important;
        line-height:1 !important;
      }
      #activeProductCard.search-card-expanded .search-card-nav .nav-text{
        display:flex !important; flex-direction:column !important; min-width:0 !important; overflow:hidden !important;
      }
      #activeProductCard.search-card-expanded .search-card-nav .nav-kicker{
        font-size:10px !important; text-transform:uppercase !important; letter-spacing:.05em !important; color:rgba(233,255,245,.72) !important; margin-bottom:2px !important;
      }
      #activeProductCard.search-card-expanded .search-card-nav .nav-name{
        display:block !important; overflow:hidden !important; text-overflow:ellipsis !important; white-space:nowrap !important; font-weight:800 !important; color:#fff !important;
      }
      @media (max-width: 760px){
        #activeProductCard.search-card-expanded{
          width:min(95vw,95vw) !important; max-width:95vw !important;
          height:min(90vh,90vh) !important; max-height:90vh !important;
