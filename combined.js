...
(() => {
  ...
  async function saveBranchSheetCurrent(index){
    ...
    if(sourceChanged){
      clearImportedProductsForBranch(index, { resetUi: appState.activeBranchIndex===index, clearHeaders:true });
      branch.sheetStatusText = 'Vinculación actualizada. Vuelve a leer fila 1 e importa los productos.';
    }else{
  ...
  async function importBranchSheet(index){
    ...
    if(hadImportedProducts){
      ...
      if(!replaceOk) return;
      clearImportedProductsForBranch(index, { resetUi: appState.activeBranchIndex===index, clearHeaders: branch.lastImportedSourceSignature !== currentSourceSignature });
    }
