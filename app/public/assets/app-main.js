...
function zoomLayout(factor, center){
  ...
  function openAuthModal(message=''){
    ...
    if(loginUsername && !loginUsername.value) loginUsername.value = authRole?.value === 'viewer' ? '' : 'admin';
  ...
  document.addEventListener('keydown', e => {
    ...
    if(isTyping) return;
    if(key === 'z' && !e.shiftKey){
      if(appState.screen === 'layout'){ e.preventDefault(); undoHistory('layout'); }
      else if(appState.screen === 'racks'){ e.preventDefault(); undoHistory('racks'); }
    }
    if(key === 'y' || (key === 'z' && e.shiftKey)){
      if(appState.screen === 'layout'){ e.preventDefault(); redoHistory('layout'); }
      else if(appState.screen === 'racks'){ e.preventDefault(); redoHistory('racks'); }
    }
  });
  renderCurrentScreen();
      closeAuthModal();
      showToast(mode === 'register' ? `Cuenta creada. Código: ${appState.auth.companyCode || '-'}` : 'Sesión iniciada correctamente.', 'success', 2600);
      return true;
    }catch(err){
      const msg = err.message || 'Error al iniciar sesión';
      if(authStatus) authStatus.textContent = msg;
      showToast(msg, 'error', 3400);
      return false;
    }
  }
...
console.info('*** REHYDRATION + SESSION RETRY FIX ACTIVE ***');
  ...
  if(btnToggleAuthMode){ const __toggleAuthMode = () => setAuthMode((authMode?.value || 'login') === 'register' ? 'login' : 'register'); btnToggleAuthMode.onclick = __toggleAuthMode; btnToggleAuthMode.addEventListener('click', __toggleAuthMode); }
