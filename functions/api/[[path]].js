const BUILD_MARK = 'cloudflare-v119-openings-zone-guards';
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/login' && request.method === 'POST') {
      ...
      // Migra hashes SHA-256 heredados a PBKDF2 después de un inicio de sesión válido.
    ...
    if (path === '/app-state' && request.method === 'GET') {
      ...
      const stored = await getStoredAppState(env.DB, session.company_id);
      return withJson({
        ok: true,
        state: stored || (await buildFallbackAppState(env.DB, session.company_id)),
        build: BUILD_MARK
      });
    }
    ...
    console.error('[WMS API]', request.method, path, err);
    return withJson({
      ok: false,
...
async function ensureSchema(db, env) {
  ...
  const defaultAdminUser = String(env.ADMIN_USERNAME || '').trim();
  const defaultAdminPassword = String(env.ADMIN_PASSWORD || '');
  ...
  const shouldSeedDefaultAdmin = Boolean(defaultAdminUser && defaultAdminPassword);
  ...
  if (shouldSeedDefaultAdmin) {
    const existingSeedUser = await first(
      db.prepare('SELECT id, password_hash FROM users WHERE username = ?').bind(defaultAdminUser)
    );
    const defaultHash = existingSeedUser?.password_hash || await hashPassword(defaultAdminPassword);
...
async function hashPassword(password) {
  ...
  return `${PASSWORD_KDF}$${PASSWORD_ITERATIONS}$${bufToHex(saltBytes)}$${bufToHex(derived)}`;
}
...
async function verifyPassword(stored, password) {
  ...
  if (parts.length === 4 && parts[0] === PASSWORD_KDF) {
    const iterations = Number(parts[1] || 0);
    const saltBytes = hexToBytes(parts[2]);
    if (!Number.isFinite(iterations) || iterations < 100000 || iterations > 1000000 || !saltBytes.length || !parts[3]) return false;
    const passwordKey = await crypto.subtle.importKey(
      'raw',
      new TextEncoder().encode(String(password || '')),
      'PBKDF2',
    ...
    return timingSafeHexEqual(bufToHex(derived), parts[3]);
  }

  // Compatibilidad con el formato heredado: salt$sha256(salt:password).
  if (parts.length === 2) {
    const [salt, hash] = parts;
    if (!salt || !hash) return false;
    const encoded = new TextEncoder().encode(`${salt}:${String(password || '')}`);
    const digest = await crypto.subtle.digest('SHA-256', encoded);
    return timingSafeHexEqual(bufToHex(digest), hash);
  }
...
function passwordHashNeedsUpgrade(stored) {
  ...
  return parts.length !== 4 || parts[0] !== PASSWORD_KDF || Number(parts[1] || 0) < PASSWORD_ITERATIONS;
}
